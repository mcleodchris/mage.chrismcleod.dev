...
export function generateMicropubMarkdown(micropub, postType, options = {}) {
  ...
  let content = "";
  log.debug(
    `[MicropubMarkdown] Generating markdown for postType: ${postType}, slug: ${slug}`
  );
  log.debug(`[MicropubMarkdown] Payload: ${JSON.stringify(micropub)}`);
  if (postType === "note") {
    fileDir = "notes";
    content =
      (micropub.properties.content && micropub.properties.content[0]) || "";
    frontmatter.tags = micropub.properties.category || [];
    log.debug(`[MicropubMarkdown] Note content: ${content}`);
  } else if (postType === "bookmark") {
    fileDir = "bookmarks";
    frontmatter.bookmark_of =
      (micropub.properties["bookmark-of"] &&
        micropub.properties["bookmark-of"][0]) ||
      "";
    frontmatter.tags = micropub.properties.category || [];
    frontmatter.name =
      (micropub.properties.name && micropub.properties.name[0]) || "";
    content =
      (micropub.properties.content && micropub.properties.content[0]) || "";
    log.debug(
    ...
    fileDir = "likes";
    frontmatter.like_of =
      (micropub.properties["like-of"] && micropub.properties["like-of"][0]) ||
      "";
    frontmatter.tags = micropub.properties.category || [];
    content =
      (micropub.properties.content && micropub.properties.content[0]) || "";
    log.debug(
      `[MicropubMarkdown] Like content: ${content}, like_of: ${frontmatter.like_of}`
    );
  }
  // YAML frontmatter
