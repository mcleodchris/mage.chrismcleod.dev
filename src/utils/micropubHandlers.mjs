...
export async function handleUpload(req, res) {
    ...
    try {
        if (imageFile.mimetype === "image/jpeg") {
            ...
            if (pngMetadata.tEXt?.creation_time) {
    ...
    // upload to Bunny
    await uploadToBunnyStorage(req.bunnyStorageConfig, imagePath, `${uploadPath}/${filename}`);
    ...
    for (const format in metadata) {
        for (const size of metadata[format]) {
            ...
            log.info(`Resized image uploaded to Bunny Storage: ${format} :: ${size.width}`);
            // await uploadToAzureBlobStorage(
            //   req.storageContainerClient,
            //   size.outputPath,
            //   `${uploadPath}/resized/${size.filename}`
