...
} from "../utils/micropubHandlers.mjs";
...
router.get("/", (req, res) => {
  ...
  return res
    .status(400)
    .json({ error: "invalid_request", error_description: "Unknown query" });
});
...
router.post(
  ...
  async (req, res) => {
    ...
    if (
      ...
      log.warn(
        `[Micropub] Invalid Micropub structure: ${JSON.stringify(payload)}`
      );
      return res.status(400).json({
        error: "invalid_request",
    ...
    // Date
    const published =
      (payload.properties.published && payload.properties.published[0]) ||
      new Date().toISOString();
    // Title
    const title = payload.properties.name
      ? payload.properties.name[0]
      : undefined;
    ...
    const dateTime = `${dateObj.getFullYear()}${pad(
      dateObj.getMonth() + 1
    )}${pad(dateObj.getDate())}${pad(dateObj.getHours())}${pad(
      dateObj.getMinutes()
    )}`;
    const postUrl = `${
